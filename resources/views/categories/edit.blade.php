...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h3 class="text-xl font-bold text-slate-100">Ubah Kategori</h3>
            <p class="text-sm text-slate-400">Sesuaikan informasi kategori keuangan.</p>
        </div>
    </div>
    ...
    <div class="bg-slate-950/40 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
        <form method="POST" action="{{ route('categories.update', $category) }}" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input id="name" type="text" name="name" value="{{ old('name', $category->name) }}" required placeholder="Misal: Eco-Plumbing Service, Biaya Listrik & Air" class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm placeholder-slate-650 transition duration-200 outline-none text-slate-200">
                @error('name')
