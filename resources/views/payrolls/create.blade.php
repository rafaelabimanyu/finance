...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h3 class="text-xl font-bold text-slate-100">Catat Payroll Baru</h3>
            <p class="text-sm text-slate-400">Rekam transaksi penggajian karyawan atau teknisi lapangan.</p>
        </div>
    </div>
    ...
    <div class="bg-slate-950/40 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
        <form method="POST" action="{{ route('payrolls.store') }}" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input id="role_type" type="text" name="role_type" value="{{ old('role_type') }}" required placeholder="Misal: Gaji Teknisi Lapangan Eco-Plumbing, Gaji Staf Administrasi" class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm placeholder-slate-650 transition duration-200 outline-none text-slate-200">
                @error('role_type')
                    <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input id="base_salary" type="number" step="0.01" name="base_salary" value="{{ old('base_salary') }}" required placeholder="Misal: 5500000" class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm placeholder-slate-650 transition duration-200 outline-none text-slate-200">
                    @error('base_salary')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input id="period" type="text" name="period" value="{{ old('period', now()->format('Y-m')) }}" required placeholder="Contoh: 2026-07" class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm placeholder-slate-650 transition duration-200 outline-none text-slate-200">
                    @error('period')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-2">
                ...
                <select id="status" name="status" required class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-850 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm transition duration-200 outline-none text-slate-200 cursor-pointer">
                    <option value="pending" {{ old('status') === 'pending' ? 'selected' : '' }}>Tertunda (Pending)</option>
                    <option value="paid" {{ old('status') === 'paid' ? 'selected' : '' }}>Lunas Terbayar (Paid)</option>
                </select>
                @error('status')
                    <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                @enderror
