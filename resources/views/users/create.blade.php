...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h3 class="text-xl font-bold text-slate-100">Daftarkan Pengguna Baru</h3>
            <p class="text-sm text-slate-400">Buat akun akses baru untuk Owner atau Admin sistem.</p>
        </div>
    </div>
    ...
    <div class="bg-slate-950/40 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
        <form method="POST" action="{{ route('users.store') }}" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input id="name" type="text" name="name" value="{{ old('name') }}" required placeholder="Misal: Ardy Wijaya" class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm placeholder-slate-650 transition duration-200 outline-none text-slate-200">
                @error('name')
            ...
            <div class="space-y-2">
                ...
                <input id="email" type="email" name="email" value="{{ old('email') }}" required placeholder="nama@jj-group.id" class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm placeholder-slate-650 transition duration-200 outline-none text-slate-200">
                @error('email')
                    <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <select id="role" name="role" required class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm transition duration-200 outline-none text-slate-200 cursor-pointer">
                    ...
                    <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Admin (Pengelola Data Harian)</option>
                    <option value="owner" {{ old('role') === 'owner' ? 'selected' : '' }}>Owner (Pemilik Sistem)</option>
                </select>
                @error('role')
                    <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input id="password" type="password" name="password" required placeholder="Minimal 8 karakter" class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm placeholder-slate-650 transition duration-200 outline-none text-slate-200">
                    @error('password')
                        <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-2">
                    <label for="password_confirmation" class="text-xs font-semibold text-slate-300 uppercase tracking-wider">Konfirmasi Kata Sandi</label>
                    <input id="password_confirmation" type="password" name="password_confirmation" required placeholder="Ulangi kata sandi" class="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-sm placeholder-slate-650 transition duration-200 outline-none text-slate-200">
                </div>
            </div>
